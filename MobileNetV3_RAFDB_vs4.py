...
class MobileNetV3_Enhanced(nn.Module):
    def __init__(self, num_classes=7):
        ...
        backbone_output_features = 1280  # Thay vì 960
        
        self.global_pool = nn.AdaptiveAvgPool2d(1)
        
        self.channel_attention = nn.Sequential(
            nn.Linear(backbone_output_features, backbone_output_features // 8),  # nn.Linear(1280, 160)
            nn.Hardswish(inplace=True),
            nn.Dropout(0.1),
            nn.Linear(backbone_output_features // 8, backbone_output_features),  # nn.Linear(160, 1280)
            nn.Sigmoid()
        )
        
        self.dropout1 = nn.Dropout(0.25)
        self.fc1 = nn.Linear(backbone_output_features, 512)  # nn.Linear(1280, 512)
...
# Tệp log
metrics_log_file = "MobileNetV3_RAFDB_vs4_metrics_log.csv"
confusion_matrix_log_file = "MobileNetV3_RAFDB_vs4_confusion_matrix_log.csv"
...
for epoch in range(1, NUM_EPOCHS + 1):
    ...
    test_acc, test_f1, test_prec, test_rec = compute_metrics(
        all_labels_test, all_preds_test  # Hoặc all_preds_test, all_labels_test
    )
    ...
    if val_f1 > best_f1:
        ...
        torch.save(model.state_dict(), "MobileNetV3_RAFDB_vs4_f1.pth")
    else:
        epochs_no_improve += 1
    ...
    if val_acc > best_acc:
        ...
        torch.save(model.state_dict(), "MobileNetV3_RAFDB_vs4_acc.pth")

    if val_loss < best_loss:
        best_loss = val_loss
        torch.save(model.state_dict(), "MobileNetV3_RAFDB_vs4_loss.pth")
