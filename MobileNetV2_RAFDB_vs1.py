...
# Data loaders với cải tiến hiệu suất
...
# Kích hoạt cuDNN benchmark
torch.backends.cudnn.benchmark = True

# Sử dụng torch.compile nếu có trong phiên bản PyTorch
...
# Loss, Optimizer, Scheduler - GIỮ NGUYÊN PHẦN NÀY
...
def validate_model(model, dataloader, criterion):
    ...
    val_acc, val_f1, val_prec, val_rec = compute_metrics(all_labels, all_preds)
    
    return val_loss, val_acc, val_f1, val_prec, val_rec, all_preds, all_labels
...
for epoch in range(1, NUM_EPOCHS + 1):
    ...
    for inputs, labels in train_loader:
        ...
        # Đặt gradient về None thay vì zero
        for param in model.parameters():
            param.grad = None
        
        # Sử dụng autocast cho AMP
        with autocast():
            outputs = model(inputs)
            loss = criterion(outputs, labels)
        
        # Scale loss và thực hiện backpropagation
    ...
    # Tính các metrics training
    train_loss = running_loss / len(train_loader.dataset)
    train_acc, train_f1, train_prec, train_rec = compute_metrics(all_labels, all_preds)
    
    # Validation phase
    val_loss, val_acc, val_f1, val_prec, val_rec, val_preds, val_labels = validate_model(model, val_loader, criterion)
    
    # Test phase
    test_loss, test_acc, test_f1, test_prec, test_rec, test_preds, test_labels = validate_model(model, test_loader, criterion)
    ...
    with open(metrics_log_file, "a") as f:
        log_line = (
            ...
            f"{train_acc:.4f},{train_loss:.4f},{train_f1:.4f},{train_prec:.4f},{train_rec:.4f}\n"
        )
        f.write(log_line)
    ...
    if val_f1 > best_f1:
        ...
        # Lưu model state dict và các trạng thái khác
        torch.save({
            'epoch': epoch,
    ...
    if val_loss < best_loss:
        best_loss = val_loss
        print("Model improved (Loss). Saving best model weights.")
        torch.save({
            'epoch': epoch,
            'model_state_dict': model.state_dict(),
            'optimizer_state_dict': optimizer.state_dict(),
