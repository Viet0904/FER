...
VAL_SIZE = 0.2
TARGET_SIZE = (224, 224)  # MobileNet hỗ trợ 224x224
...
def visualize_combined_class_distribution(
    ...
    )  # Giả sử các nhãn giống nhau giữa các tập
    ...
    plt.bar(
        index + bar_width, val_counts, bar_width, label="Validation", color="salmon"
    )
    plt.bar(
        index + 2 * bar_width, test_counts, bar_width, label="Test", color="lightgreen"
    )
...
# Xây dựng mô hình MobileNet (MobileNetV1)
class MobileNet_Model(nn.Module):
    def __init__(self, num_classes):
        super(MobileNet_Model, self).__init__()
        self.backbone = timm.create_model(
            "mobilenetv1_100", pretrained=True, num_classes=num_classes
        )
...
# Tệp log
metrics_log_file = "MobileNet_RAFDB_metrics_log.csv"
confusion_matrix_log_file = "MobileNet_RAFDB_confusion_matrix_log.csv"
...
for epoch in range(1, NUM_EPOCHS + 1):
    ...
    if val_f1 > best_f1:
        ...
        torch.save(model.state_dict(), "MobileNet_RAFDB_f1.pth")
    else:
        epochs_no_improve += 1
    ...
    if val_acc > best_acc:
        ...
        torch.save(model.state_dict(), "MobileNet_RAFDB_acc.pth")

    if val_loss < best_loss:
        best_loss = val_loss
        torch.save(model.state_dict(), "MobileNet_RAFDB_loss.pth")
