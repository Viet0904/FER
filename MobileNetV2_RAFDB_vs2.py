...
class EnhancedMobileNetV2(nn.Module):
    ...
    def forward(self, x):
        ...
        x = features[-1]  # Lấy lớp đặc trưng cuối cùng
        
        # Áp dụng attention và squeeze-excitation
        x = self.attention(x)
        x = self.se(x)
...
# Tệp log
metrics_log_file = "EnhancedMobileNetV2_RAFDB_metrics_log.csv"
confusion_matrix_log_file = "EnhancedMobileNetV2_RAFDB_confusion_matrix_log.csv"
...
for epoch in range(1, NUM_EPOCHS + 1):
    ...
    if val_f1 > best_f1:
        ...
        torch.save(model.state_dict(), "EnhancedMobileNetV2_RAFDB_f1.pth")
    else:
        epochs_no_improve += 1
    ...
    if val_acc > best_acc:
        ...
        torch.save(model.state_dict(), "EnhancedMobileNetV2_RAFDB_acc.pth")

    if val_loss < best_loss:
        best_loss = val_loss
        torch.save(model.state_dict(), "EnhancedMobileNetV2_RAFDB_loss.pth")
