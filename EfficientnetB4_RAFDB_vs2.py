...
DROPOUT_RATE = 0.5  # Thêm dropout_rate làm hằng số
...
class FineTunedEfficientNetB4(nn.Module):
    def __init__(self, num_classes=7, dropout_rate=0.5):  # Thêm dropout_rate làm tham số
        ...
        self.classifier = nn.Sequential(
            nn.Dropout(dropout_rate),          # Dropout đầu tiên
            nn.Linear(num_ftrs, 512),          # Dense (1792 -> 512)
            nn.BatchNorm1d(512),               # BatchNormalization
            nn.ReLU(),                         # ReLU activation
            nn.Dropout(dropout_rate),          # Dropout thứ hai
            nn.Linear(512, num_classes)        # Dense (512 -> 7)
        )
