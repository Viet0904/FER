...
VAL_SIZE = 0.2
TARGET_SIZE = (224, 224)  # MobileNetV1 hỗ trợ 224x224
...
# Định nghĩa mô hình MobileNetV1 với attention
class MobileNet_Attention(nn.Module):
    def __init__(self, num_classes):
        super(MobileNet_Attention, self).__init__()
        # Load MobileNetV1 từ timm
        self.backbone = timm.create_model("mobilenetv1_100", pretrained=True)
        
        # Lấy số kênh của lớp đặc trưng cuối cùng trước classifier
        self.feature_channels = 1024  # MobileNetV1 có 1024 kênh trước lớp fully connected
        
        # Thêm SEModule sau lớp đặc trưng cuối cùng của backbone
        self.attention = SEModule(self.feature_channels)
        
        # Thêm Batch Normalization và Dropout
        self.bn = nn.BatchNorm2d(self.feature_channels)
    ...
    def forward(self, x):
        ...
        x = self.backbone.features(x)  # Lấy đầu ra từ các lớp convolution
        
        # Áp dụng attention
        x = self.attention(x)
        
        # Pooling, normalization và dropout
        x = self.avgpool(x)
        x = self.bn(x)
        x = self.dropout(x)
...
# Khởi tạo mô hình MobileNetV1
model = MobileNet_Attention(num_classes=NUM_CLASSES)
...
# Tệp log
metrics_log_file = "MobileNet_RAFDB_vs1_metrics_log.csv"
confusion_matrix_log_file = "MobileNet_RAFDB_vs1_confusion_matrix_log.csv"
...
for epoch in range(1, NUM_EPOCHS + 1):
    ...
    if val_f1 > best_f1:
        ...
        torch.save(model.state_dict(), "MobileNet_RAFDB_vs1_f1.pth")
    else:
        epochs_no_improve += 1
    ...
    if val_acc > best_acc:
        ...
        torch.save(model.state_dict(), "MobileNet_RAFDB_vs1_acc.pth")

    if val_loss < best_loss:
        best_loss = val_loss
        torch.save(model.state_dict(), "MobileNet_RAFDB_vs1_loss.pth")
